...
function defaultSerializeFunction(
  ...
  for (prop in session) {
    if (prop === 'cookie') {
      ...
      // 因为直接继承源代码MongoStore发现根本不起效,只能直接把源代码搬过来使用了
      // 下面的这个注释需要在eslintrc.js中加上@typescript-eslint/ban-ts-comment这句话才能过,要不然lint代码的时候会报错
      // @ts-ignore
...
export class SessionMongoStore extends session.Store {
  ...
  static create(options: ConnectMongoOptions): SessionMongoStore {
