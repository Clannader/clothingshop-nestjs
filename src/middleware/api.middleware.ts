  ...
  async use(req: Request, res: Response, next: NextFunction) {
    if (!Utils.isHasJsonHeader(req)) {
      return res.send({
        ...
        msg: this.globalService.serverLang('缺少content-type头信息', 'user.invContent')
      })
    }
    if (!Utils.isHasRequestedHeader(req)) {
      return res.send({
        code: CodeEnum.INVALID_HEADERS,
        msg: this.globalService.serverLang('缺少x-requested-with头信息', 'user.invRequest')
      })
    }
    ...
    if (adminSession.isFirstLogin) {
      ...
      return res.send({
        ...
        msg: this.globalService.serverLang('用户第一次登录，请修改密码', 'user.isFirstLogin')
      })
    }
    // 每次访问延长用户有效期时间
