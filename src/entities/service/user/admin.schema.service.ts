...
export class AdminSchemaService {
  ...
  async loginSystem(adminId: string): Promise<LoginResult> {
    ...
    // 通过上面的条件查询用户表
    let [err, adminInfo] = await this.adminModel.findOne(where)
      .lean().then(resp => [null, resp]).catch(err => [err])
    if (err) {
      return Promise.reject({
        message: err.message,
    ...
    if (Utils.isEmpty(adminInfo)) {
      ...
      if (Utils.isSupervisor({adminId, orgRights: []})) {
        const [createErr, superInfo] = await this.adminModel.create(Utils.getSuper())
          .then(resp => [null, resp]).catch(err => [err])
        if (createErr) {
          return Promise.reject({
